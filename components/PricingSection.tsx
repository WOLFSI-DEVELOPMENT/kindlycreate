    ...
    <div className="w-full h-full bg-[#FAFAFA] overflow-y-auto p-8 flex items-center justify-center font-inter text-slate-900">
      ...
      <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-blue-500/10 relative flex flex-col transform md:-translate-y-4 ring-1 ring-black/5">
          <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest shadow-lg">
            Most Popular
          </div>
          <div className="flex items-center justify-between mb-4">
          ...
          <p className="text-sm text-slate-500 mb-8 leading-relaxed">For growing teams that need more power and flexibility.</p>
          <button className="w-full py-2.5 rounded-xl bg-slate-900 text-white font-medium hover:bg-slate-800 transition-all active:scale-95 mb-8 shadow-md shadow-slate-200 text-sm">
            Start Free Trial
          </button>
          <div className="space-y-3 flex-1">
            <Feature text="Unlimited Users" highlighted />
            <Feature text="Unlimited Projects" highlighted />
            <Feature text="Priority Support" highlighted />
            <Feature text="Analytics Dashboard" highlighted />
          </div>
        </div>
...
const Feature = ({ text, highlighted = false }: { text: string, highlighted?: boolean }) => (
  <div className="flex items-center gap-3">
    <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${highlighted ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-500'}`}>
